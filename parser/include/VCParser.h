...
//Returns the FN value (aka name)
char * getFNValue (char * fileName);

//Finds the length of OP list (int)
int lengthOfOPList (char *fileName);

//Given a filename, it will create a JSON object (string) of the whole card
char * fileToJSON (char *fileName);

//Sends back an JSON array string which has all the length of param list
...
//Upon sending in a property struct, constructs a JSON object (string) with param name and value
